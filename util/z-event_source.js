...
(function (global) {
  ...
  function zEventSource(url, options) {
    if (!options) {
      options = {};
    }
    ...
    var heartbeatTimeout = getDuration(500000, 0);
      ...
      if (currentState === CONNECTING) {
        ...
        if (status === 200 && contentTypeRegExp.test(contentType)) {
          ...
          if (status !== 0 && (status !== 200 || contentType !== "")) {
            var message = "";
            if (status !== 200) {
              message = "zEventSource's response has a status " + status + " " + statusText.replace(/\s+/g, " ") + " that is not 200. Aborting the connection.";
            } else {
              message = "zEventSource's response has a Content-Type specifying an unsupported type: " + contentType.replace(/\s+/g, " ") + ". Aborting the connection.";
            }
            setTimeout(function () {
    ...
    onTimeout = function () {
      ...
      if ("setRequestHeader" in xhr) {
        ...
        if (options.headers && options.headers.forEach) {
  ...
  zEventSource.prototype = new F();
  F.call(zEventSource);
  if (isCORSSupported) {
    zEventSource.prototype.withCredentials = undefined;
  }

  var iszEventSourceSupported = function () {
    // Opera 12 fails this test, but this is fine.
    return global.zEventSource != undefined && ("withCredentials" in global.zEventSource.prototype);
  };

  if (Transport != undefined && (global.zEventSource == undefined || (isCORSSupported && !iszEventSourceSupported()))) {
    // Why replace a native zEventSource ?
    // https://bugzilla.mozilla.org/show_bug.cgi?id=444328
    // https://bugzilla.mozilla.org/show_bug.cgi?id=831392
    // https://code.google.com/p/chromium/issues/detail?id=260144
    // https://code.google.com/p/chromium/issues/detail?id=225654
    // ...
    global.NativezEventSource = global.zEventSource;
    global.zEventSource = zEventSource;
  }
